...
import 'package:anime_app/utils/utils.dart';
...
//getting category images
  Future<List<WallpaperModel>> getpics(int page, int limit) async {
    try {
      final result = await http.get(
        Uri.https(
          ...
          '/v3/images',
          {
            'page[limit]': '$limit',
            'page[offset]':
                '${page * limit}', // Calculate the offset based on the page and limit
      ...
      if (result.statusCode == HttpStatus.ok) {
        ...
        if (data is List) {
          return data
              .map<WallpaperModel>((item) => WallpaperModel.fromMap(item))
              .toList();
        }
      }
    } catch (e) {
  ...
  Future<List<CategoryModel>> getCategory() async {
    try {
      final result = await http.get(
        Uri.https('api.nekosapi.com', '/v3/images/tags', {
          'filter[ageRating]': 'sfw',
        }),
        headers: {
          'Accept': 'application/json',
  ...
  Future<void> fetchCategoryImages(String categoryId) async {
    ...
    try {
      final newCategoryImages =
          await getCategoryImages(categoryId, currentPage, pageSize);
  ...
  Future<List<WallpaperModel>> getCategoryImages(
